...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    private float _accelerationForce = 25f;
    ...
    private void SetStates()
    {
        ...
        {
            _ when _movementDirection == Vector3.zero && isGrounded && !isSliding =>
                PlayerState.Idle,
            _ when _movementDirection != Vector3.zero && isGrounded && !isSliding =>
                PlayerState.Move,
            _ when _movementDirection != Vector3.zero && isGrounded && isSliding =>
    ...
    {
        _movementDirection =
            _orientationTransform.forward * _verticalInput
        ...
        _playerRigidBody.AddForce(
            _movementDirection.normalized * _accelerationForce * forceMultiplier,
            ForceMode.Force
        );
    }
    ...
    public bool CanCatChase()
    {
        ...
        )
        {
            if (hit.collider.gameObject.layer == LayerMask.NameToLayer(Consts.Layers.FLOOR_LAYER))
            {
                ...
                hit.collider.gameObject.layer == LayerMask.NameToLayer(Consts.Layers.GROUND_LAYER)
            )
            {
                return false;
            }
